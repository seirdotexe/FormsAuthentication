  ...
  static deserialize(buffer) {
    ...
    if (buffer.readUint8(position++) === 1) {
      ...
      if (buffer.readUint8(position++) === 254) {
        ...
        const name = decoder.write(buffer.subarray(position, position + nameLength)); position += nameLength;

        const userDataLength = buffer.readUint8(position++);
        const userData = decoder.write(buffer.subarray(position, position + userDataLength)); position += userDataLength;

        const cookiePathLength = buffer.readUint8(position++);
        const cookiePath = decoder.write(buffer.subarray(position, position + cookiePathLength)); position += cookiePathLength;
